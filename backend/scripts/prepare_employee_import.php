...
$input = $argv[1] ?? null;
$output = $argv[2] ?? null;
$categoryName = trim((string) ($argv[3] ?? ''));
$emailPrefix = trim((string) ($argv[4] ?? Str::slug($categoryName)));
if (!$input || !$output || !$categoryName || !is_file($input)) {
    fwrite(STDERR, "Usage: php prepare_employee_import.php input.xlsx output.xlsx category [email-prefix]\n");
    exit(1);
}

$category = JobCategory::whereRaw('LOWER(name) = ?', [mb_strtolower($categoryName)])->first();
if (!$category) {
    fwrite(STDERR, "Category '{$categoryName}' does not exist in the database.\n");
    exit(1);
}
...
$nextGenerated = 9;
$generatedPhones = 0;
$invalidPhonesReplaced = 0;
$duplicatePhonesReplaced = 0;
$duplicatesRemoved = 0;
$missingDates = 0;
$correctedDates = 0;
...
foreach ($rows as $offset => $row) {
    ...
    $validPhone = is_string($phone) && preg_match('/^\+2519\d{8}$/', $phone) === 1;
    $phoneAlreadyUsed = $validPhone && isset($usedPhones[$phone]);
    if (!$validPhone || $phoneAlreadyUsed) {
        if (!$validPhone) {
            $invalidPhonesReplaced++;
        } else {
            $duplicatePhonesReplaced++;
        }
        do {
    ...
    do {
        $email = mb_strtolower($emailPrefix) . '.import.' . str_pad((string) $sequence++, 3, '0', STR_PAD_LEFT) . '@alefdelta.com';
...
$summary->fromArray([
    ['ALEF DELTA EMPLOYEE IMPORT PREPARATION', ''],
    ['Original spreadsheet rows', count($rows)],
    ['Exact duplicate rows removed', $duplicatesRemoved],
    ['Distinct employees prepared', count($prepared)],
    ['Generated/replaced phone numbers', $generatedPhones],
    ['Invalid/missing phones replaced', $invalidPhonesReplaced],
    ['Duplicate phones replaced', $duplicatePhonesReplaced],
...
$summary->getColumnDimension('A')->setWidth(45);
...
echo json_encode([
    ...
    'generated_phones' => $generatedPhones,
    'invalid_or_missing_phones_replaced' => $invalidPhonesReplaced,
    'duplicate_phones_replaced' => $duplicatePhonesReplaced,
